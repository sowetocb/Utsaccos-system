...
public class LoanApplication {
    ...
    @Column(name = "EmergencyReason", length = 500)
    private String emergencyReason;
    @Column(name = "SupportingDocument", length = 500)
    private String supportingDocument;
    @Column(name = "DocumentVerified")
    private Boolean documentVerified = false;
    @Column(name = "PhoneVerified")
    private Boolean phoneVerified = false;
    @Column(name = "VerificationNotes", length = 500)
    private String verificationNotes;
    @Column(name = "GuarantorIdNumber", length = 50)
    private String guarantorIdNumber;
    @Column(name = "GuarantorName", length = 100)
    private String guarantorName;
    @Column(name = "GuarantorPhone", length = 20)
    private String guarantorPhone;
    @PrePersist
    protected void onCreate() {
        if (createdDate == null) createdDate = LocalDateTime.now();
        if (appliedDate == null) appliedDate = LocalDateTime.now();
        if (documentVerified == null) documentVerified = false;
